...
static const char *StrOcenType(DefineType type, char* name)
{
    switch (type)
    {
...
static const char *StrOcenDf(DefineType type)
{
    switch (type)
    {
        case UNKNOWN:     return "const";
        case GUARD:       return "const";
        case MACRO:       return "def";
        case INT:         return "const";
        case INT_MATH:    return "const";
        case LONG:        return "const";
        case LONG_MATH:   return "const";
        case FLOAT:       return "const";
        case FLOAT_MATH:  return "const";
        case DOUBLE:      return "const";
        case DOUBLE_MATH: return "const";
        case CHAR:        return "const";
        case STRING:      return "const";
        case COLOR:       return "const";
    }
    return "";
}

static const char *GetOcenType(char* ret, char* _name) {
    int n = strlen(_name);
    int is_pt = 0;
    int is_arr = 0;
    int i = 0;
    ...
    else if(strcmp("char", tname) == 0) strcpy(pt, "char"), pt+=4;
    else if(strcmp("const void", tname) == 0) if(pt-name > 0) strcpy(pt-1, "untyped_ptr"),pt+=11; else strcpy(pt, "u8"),pt+=2;
    else if(strcmp("void", tname) == 0) if(pt-name > 0) strcpy(pt-1, "untyped_ptr"),pt+=11; else strcpy(pt, "u8"),pt+=2;
    else {
        int j = 0;
        /*if(strcmp("const ", tname) == 0) j = 6;*/
        strcpy(pt, tname+j),pt+=strlen(tname+j);
    }
    if(is_arr) {
        *(pt) = ';';
        *(pt+1) = ' ';
...
static void ExportParsedData(const char *fileName, int format)
{
    ...
    switch (format)
    {
        ...
        //./raylib_parser -i raylib.h -o raylib.oc -f CODE
        {
            ...
            for (int i = 0; i < defineCount; i++)
            {
                switch(defines[i].type) {
                    ...
                    case MACRO:
                        break;
                    default: {
                        const char* dname = StrOcenDf(defines[i].type);
                        fprintf(outFile, "[extern] %s %s: %s\n", dname, defines[i].name, StrOcenType(defines[i].type, defines[i].value));
                     }
                }
            ...
            for (int i = 0; i < structCount; i++)
            {
                fprintf(outFile, "[extern] struct %s {\n", structs[i].name);
                /*fprintf(outFile, "  Name: %s\n", structs[i].name);*/
                /*fprintf(outFile, "  Description: %s\n", structs[i].desc);*/
                for (int f = 0; f < structs[i].fieldCount; f++)
                {
                    memset(ret, 0, 32);
                    fprintf(outFile, "      %s: %s ", structs[i].fieldName[f], GetOcenType(ret, structs[i].fieldType[f]));
                    /*if (structs[i].fieldDesc[f][0]) fprintf(outFile, "      // %s\n", structs[i].fieldDesc[f]);*/
                    /*else fprintf(outFile, "\n");*/
                    fprintf(outFile, "\n");
                }
            ...
            for (int i = 0; i < callbackCount; i++)
            {
                fprintf(outFile, "typedef %s = fn(", callbacks[i].name);
                /*fprintf(outFile, "Callback %03i: %s() (%i input parameters)\n", i + 1, callbacks[i].name, callbacks[i].paramCount);*/
                /*fprintf(outFile, "  Name: %s\n", callbacks[i].name);*/
                /*fprintf(outFile, "  Return type: %s\n", callbacks[i].retType);*/
                /*fprintf(outFile, "  Description: %s\n", callbacks[i].desc);*/
                for (int p = 0; p < callbacks[i].paramCount; p++) {
                    ...
                    if((p+1) < callbacks[i].paramCount)
                        /*fprintf(outFile, "%s: %s, ", callbacks[i].paramName[p], GetOcenType(ret, callbacks[i].paramType[p]));*/
                        fprintf(outFile, "%s, ", GetOcenType(ret, callbacks[i].paramType[p]));
                    else
                        /*fprintf(outFile, "%s: %s", callbacks[i].paramName[p], GetOcenType(ret, callbacks[i].paramType[p]));*/
                        fprintf(outFile, "%s", GetOcenType(ret, callbacks[i].paramType[p]));
                }
                if(callbacks[i].retType[0] != 0) {
                    memset(ret, 0, 32);
                    fprintf(outFile, "): %s\n", GetOcenType(ret, callbacks[i].retType));
                } else fprintf(outFile, ")\n");
                /*if (callbacks[i].paramCount == 0) fprintf(outFile, "  No input parameters\n");*/
            }
            ...
            for (int i = 0; i < funcCount; i++)
            {
                fprintf(outFile, "[extern] def %s(", funcs[i].name);
                /*fprintf(outFile, "  Name: %s\n", funcs[i].name);*/
                /*fprintf(outFile, "  Return type: %s\n", funcs[i].retType);*/
                /*fprintf(outFile, "  Description: %s\n", funcs[i].desc);*/
                for (int p = 0; p < funcs[i].paramCount; p++) {
                    memset(ret, 0, 32);
                    GetOcenType(ret, funcs[i].paramType[p]);
                    if(strcmp(ret, "...")==0)
                ...
                if(funcs[i].retType[0] != 0) {
                    memset(ret, 0, 32);
                    fprintf(outFile, "): %s\n", GetOcenType(ret, funcs[i].retType));
                } else fprintf(outFile, ")\n");
            }
        } break;
        default: break;
